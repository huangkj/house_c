 ...
 * <p>
 * Editor: Ybao 2016/7/31 email : 392579823@qq.com
 */
...
public class StereoView extends ViewGroup {
    ...
    private float mAngle = 30;// 两个item间的夹角
    ...
    private int addCount = 1;// 记录手离开屏幕后，需要新增的页面次数
     ...
     * 自动滚动事件
     */
    private static int TIME_AUTO_DELAYED = 2500;

    private Handler mHandler = new Handler();
    private Runnable mRunnable = new Runnable() {
        public void run() {
            toNext();
            mHandler.postDelayed(mRunnable, TIME_AUTO_DELAYED);
        }
    };
    ...
    public boolean isCanSliding(MotionEvent ev) {
        ...
        moveX = ev.getX();
        moveY = ev.getY();
        if (Math.abs(moveY - mDownX) > mTouchSlop
                && (Math.abs(moveY - mDownY) > (Math.abs(moveX - mDownX)))) {
            return true;
        }
        return false;
    }
    ...
    private void toPreAction(float yVelocity) {
        ...
        addPre();// 增加新的页面
        // 计算松手后滑动的item个数
        // int flingSpeedCount = (yVelocity - standerSpeed) > 0 ? (int)
        // (yVelocity - standerSpeed)
        // : 0;
     ...
     * @return
     */
    public StereoView setCanTouch(boolean canTouch) {

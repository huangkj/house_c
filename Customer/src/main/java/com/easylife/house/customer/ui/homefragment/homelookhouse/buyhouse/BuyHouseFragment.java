...
import com.easylife.house.customer.adapter.BuyHouseAdapter;
...
public class BuyHouseFragment extends MVPBaseFragment<BuyHouseContract.View, BuyHousePresenter> implements
        BuyHouseContract.View {
    ...
    private BuyHouseAdapter mAdapter;
    private TextView tvEmptyTitle;

    public static BuyHouseFragment newInstance() {
        BuyHouseFragment myFragment = new BuyHouseFragment();
        return myFragment;
    }
    ...
    public void onMessageEventMainThread(MessageEvent event) {
        if (event.MsgType == LOGIN_STATE_CHANGE) {
            if (mDao.isLogin()) {
                tvEmptyTitle.setText("暂无买房成交");
                tvEmptyTitle.setTextColor(getResources().getColor(R.color.black));
                mPresenter.requestLookHouse(mDao.getLoginCache().userCode, mDao.getLoginCache().token, "2");
            } else {
                ...
                tvEmptyTitle.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        startActivityForResult(new Intent(getActivity(), LoginByVerifyCodeActivity.class), 4);
                    }
                });
            }
        }
    ...
    public void initViews() {
        ...
        mAdapter = new BuyHouseAdapter(R.layout.net_appointment_item, netList);
        ...
        mAdapter.setItemChildOnclickListenear(new BuyHouseAdapter.ItemChildOnclickListenear() {
            @Override
...
//                CustomerUtils.showTip(getActivity(),"取消预约");
            }
        });

...
public class StatusBarUtils {
     ...
     *
     * @param activity       需要设置的activity
     * @param statusBarAlpha 状态栏透明度   状态栏完全不透明时为黑色
     * @param needOffsetView 需要向下偏移的 View
     */
     ...
     * 为头部是 ImageView 的界面设置状态栏透明
     *
     * @param color          状态栏颜色 包含透明度 如Color.argb(0,255,255,255)
     * @param activity       需要设置的activity
     * @param needOffsetView 需要向下偏移的 View
     */
    public static void setTranslucentForImageView2(Activity activity, @ColorInt int color,
                                                   View needOffsetView) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparentForWindow(activity);
        addTranslucentView2(activity, color);
        if (needOffsetView != null) {
            Object haveSetOffset = needOffsetView.getTag(TAG_KEY_HAVE_SET_OFFSET);
            if (haveSetOffset != null && (Boolean) haveSetOffset) {
                return;
     ...
     * @param  color 状态栏颜色 包含透明度 如Color.argb(0,255,255,255)
     */
    private static void addTranslucentView2(Activity activity, @ColorInt int color) {
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        ...
        if (fakeTranslucentView != null) {
            ...
            contentView.addView(createTranslucentStatusBarView2(activity, color));
        }
    }
     ...
     * @return 半透明 View
     */
    private static View createTranslucentStatusBarView2(Activity activity, int color) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));

...
public class FlowViewGroup extends RadioGroup {
    ...
    public void addViews(int itemLayout, String[] texts, final TabClickListener listener) {
        removeAllViews();
        if (itemLayout == 0 || texts == null || texts.length == 0)
            return;
        for (int i = 0; i < texts.length; i++) {
            final int position = i;
            final String text = texts[i];
            if (!TextUtils.isEmpty(text)) {
                RadioButton tvArea = (RadioButton) LayoutInflater.from(getContext()).
                        inflate(itemLayout, this, false);
                tvArea.setText(text);
                if (listener != null)
                    tvArea.setOnClickListener(new View.OnClickListener() {
                                                  @Override
                                                  public void onClick(View v) {
                                                      listener.onTabClick(position, text);
                                                  }
                                              }
                    );
                addView(tvArea);
    ...
    public void addViewsCheckBox(int itemLayout, String[] texts, final TabClickListener listener) {
        removeAllViews();
        if (itemLayout == 0 || texts == null || texts.length == 0)
            return;
        for (int i = 0; i < texts.length; i++) {
            final int position = i;
            final String text = texts[i];
            CheckBox tvArea = (CheckBox) LayoutInflater.from(getContext()).
                    inflate(itemLayout, this, false);
            tvArea.setText(text);
            tvArea.setOnClickListener(new View.OnClickListener() {
                                          @Override
